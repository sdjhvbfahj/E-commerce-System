...
/** 方便在浏览器控制台里还原演示数据（App / 小程序上没有 window，跳过即可） */
if (typeof window !== 'undefined') {
    (window as unknown as Record<string, unknown>).__resetMockDb = resetDb;
}

/** 清掉本地持久化的「服务端」数据与登录态，用于演示前重置（移动端设置页会用到） */
export function resetMockStorage() {
    resetDb();
    storage.removeItem('eshop621-mobile-user');
    storage.removeItem('eshop621-mobile-cart');
}
